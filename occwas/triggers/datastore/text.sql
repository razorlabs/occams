...
--- Note: texts do not support choices
---

DROP FOREIGN TABLE IF EXISTS value_text_ext;
...
CREATE OR REPLACE FUNCTION value_text_mirror() RETURNS TRIGGER AS $$
  BEGIN
    CASE TG_OP
      WHEN 'INSERT' THEN
...
DROP TRIGGER IF EXISTS  value_text_mirror ON "text";


CREATE TRIGGER value_text_mirror AFTER INSERT OR UPDATE OR DELETE ON "text"
