  ...
  BEGIN
    ...
    IF     (TG_OP = 'INSERT' AND NEW.value IS NOT NULL)
        OR (TG_OP = 'UPDATE'
              ...
              AND NOT EXISTS(SELECT 1 FROM value_datetime_ext WHERE (old_db, old_id) = (SELECT current_database(), OLD.id)))
        THEN
    ...
    ELSIF  (TG_OP = 'DELETE')
      ...
      DELETE FROM value_datetime_ext WHERE (old_db, old_id) = (SELECT current_database(), OLD.id);
