...
DROP FOREIGN TABLE IF EXISTS value_decimal_ext;
  ...
  BEGIN
    ...
    IF     (TG_OP = 'INSERT' AND NEW.value IS NOT NULL)
        OR (TG_OP = 'UPDATE'
              ...
              AND NOT EXISTS(SELECT 1 FROM value_decimal_ext WHERE old_db = (SELECT current_database()) AND old_id = OLD.id))
        THEN
    ...
    ELSIF  (TG_OP = 'DELETE')
      ...
      DELETE FROM value_decimal_ext WHERE old_db = (SELECT current_database()) AND old_id = OLD.id;
...
DROP TRIGGER IF EXISTS value_decimal_mirror ON "decimal";
