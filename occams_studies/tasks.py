...
def includeme(config):
    ...
    assert 'studies.export.user' in settings, 'Must specify an export user'
    ...
    if 'studies.export.expire' in settings:
        settings['studies.export.expire'] = int(settings['studies.export.expire'])

    celery.conf.update(
        BROKER_URL=settings['studies.celery.broker.url'],
        CELERY_RESULT_BACKEND=settings['studies.celery.backend.url'],
